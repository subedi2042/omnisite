  ...
  return (
    <div className="platform-app">
      <aside className={sidebarOpen ? 'platform-sidebar is-open' : 'platform-sidebar'}>
        ...
        <button className="platform-site-switcher"><span className="platform-site-mark">{draftSite.orgName.charAt(0)}</span><span><strong>{draftSite.orgName}</strong><small>{draftSite.orgType.replace('homeservice', 'home service')} website</small></span><ChevronDown size={16} /></button>
...
function WebsiteEditor({ site, setSite }: { site: SiteData; setSite: (site: SiteData) => void }) {
  ...
  const applyTheme = (themeId: ThemePresetID) => {
    ...
    setSite({ ...preset, published: false, lastPublishedAt: site.lastPublishedAt });
    setAddingPage(false);
  };

  const updatePage = (changes: Partial<Page>) => {
    if (!activePage) return;
    setSite({ ...site, pages: site.pages.map(page => page.id === activePage.id ? { ...page, ...changes } : page) });
  };

  const updateHero = (field: 'title' | 'subtitle', value: string) => {
    if (!activePage) return;
    updatePage({ sections: activePage.sections.map(section => section.type === 'hero' ? { ...section, [field]: value } : section) });
  };

  return <main className="platform-page editor-page"><div className="platform-page-heading"><div><span>Website</span><h1>Design the complete site.</h1><p>Choose a full industry template, add pages, and verify every layout on desktop or phone. All builder features in this workspace are free.</p></div><div className="platform-saved"><Check size={15} />Draft saved</div></div><div className="advanced-layout"><aside className="advanced-panel"><div className="advanced-tabs"><button className={panel === 'pages' ? 'active' : ''} onClick={() => setPanel('pages')}><FileText size={16} />Pages</button><button className={panel === 'design' ? 'active' : ''} onClick={() => setPanel('design')}><Paintbrush size={16} />Design</button></div>{panel === 'pages' ? <><button className="advanced-add" onClick={() => setAddingPage(!addingPage)}><Plus size={14} /> Add page</button>{addingPage && <div className="page-template-menu">{pageOptions.map(title => <button key={title} onClick={() => addPage(title)}>{title}</button>)}</div>}<button className="complete-site" onClick={completeSite}><Sparkles size={15} /><span><strong>Complete my website</strong><small>Add every essential page free</small></span></button><div className="advanced-page-list">{site.pages.map(page => <button className={page.id === activePage?.id ? 'advanced-page active' : 'advanced-page'} onClick={() => setActivePageId(page.id)} key={page.id}><span><strong>{page.title}</strong><small>{page.slug}</small></span><MoreHorizontal size={16} /></button>)}</div>{activePage && <div className="page-details"><span>Page details</span><label>Page name<input value={activePage.title} onChange={event => updatePage({ title: event.target.value })} /></label><label>URL slug<input value={activePage.slug} onChange={event => updatePage({ slug: event.target.value.startsWith('/') ? event.target.value : `/${event.target.value}` })} /></label><label>Page heading<input value={activePage.sections.find(section => section.type === 'hero')?.title || ''} onChange={event => updateHero('title', event.target.value)} /></label><label>Intro text<textarea value={activePage.sections.find(section => section.type === 'hero')?.subtitle || ''} onChange={event => updateHero('subtitle', event.target.value)} /></label></div>}</> : <><div className="template-list"><span>Full website templates</span>{(['sanctuary_modern','local_authority','local_table','trusted_home_pro','modern_merchant'] as ThemePresetID[]).map(id => { const theme = THEME_MANIFESTS.find(item => item.id === id)!; return <button key={id} className={site.theme.themePreset === id ? 'active' : ''} onClick={() => applyTheme(id)}><i style={{ background: theme.colors.primary }} /><span><strong>{theme.name}</strong><small>{theme.industry}</small></span>{site.theme.themePreset === id && <Check size={15} />}</button>; })}</div><p className="template-note">Choosing a template loads its matching business type, navigation, services, hours, content, and design system.</p><div className="advanced-theme"><span>Brand colors</span><div><i style={{ background: site.theme.primaryColor }} /><i style={{ background: site.theme.accentColor }} /><i style={{ background: site.theme.backgroundColor }} /></div>{([['Primary','primaryColor'],['Accent','accentColor'],['Background','backgroundColor'],['Text','textColor']] as const).map(([label, key]) => <label key={key}><span>{label}<code>{site.theme[key]}</code></span><input aria-label={label} type="color" value={site.theme[key]} onChange={event => setSite({ ...site, theme: { ...site.theme, [key]: event.target.value } })} /></label>)}</div></>}</aside><div className="advanced-canvas"><div className="advanced-canvas-tools"><div className="viewport-switcher"><button className={viewport === 'desktop' ? 'active' : ''} onClick={() => setViewport('desktop')}><Monitor size={16} /><span>Desktop</span></button><button className={viewport === 'mobile' ? 'active' : ''} onClick={() => setViewport('mobile')}><Smartphone size={16} /><span>Phone</span></button></div><span>{activePage?.title || 'Home'} · {site.orgName}</span><button><PanelLeftClose size={16} />Hide controls</button></div><SiteMiniPreview site={site} viewport={viewport} page={activePage} /></div></div></main>;
}
